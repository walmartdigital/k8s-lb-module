...
resource "azurerm_lb_rule" "lb_rule_public" {
  ...
  frontend_ip_configuration_name = azurerm_public_ip.public_ip[0].name//"${var.cluster_name}-${var.environment}-${local.lb_ports_public[count.index].target}-${var.name_suffix}-${local.lb_ports_public[count.index].frontend}-pip-frontend"
